resource "proxmox_virtual_environment_download_file" "download" {
  ...
  url       = var.url != null ? var.url : "${local.factory_url}/image/${local.schematic_id}/${local.version}/${local.platform}-${local.arch}.${local.file_extension}" # Required: HTTP/HTTPS URL
  file_name = var.url != null ? try(var.file_name, null) : try("talos-${local.schematic_id}-${local.version}-${local.platform}-${local.arch}.img", null)              # Optional: Filename override, especially for file extension issues
}
